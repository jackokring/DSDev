...
Cvertexbuffer *init_super_shape (	int rings,    	  	  // rings
	...
	*/
	...
	for (int i = 0; i < rings; i++) {
		//TODO: Wrong 4096 circ
		a1 = 0;//pow(fabs(cos(m*phi/4)/a),n2);
        a2 = 0;//pow(fabs(sin(m*phi/4)/b),n3);
        r2 = 0;//pow(a1+a2,-1/n1) ;
        r2 *= radius;
		...
		for (int j = 0; j <= bands; j++) {
			...
			a1 = 0;//pow(fabs(cos(m*theta/4)/a),n2);
			a2 = 0;//pow(fabs(sin(m*theta/4)/b),n3);
			r1 = 0;//pow(a1+a2,-1/n1) ;
			r1 *= radius;
			
			
			float x = r1  * cosLerp(theta) * r2* cosLerp(phi);
			float y = r1  * sinLerp(theta) * r2* cosLerp(phi);
			float z = r2  * sinLerp(phi);
							  ...
							  int vscale)
{
	...
	for (int i = 0 ; i < rings; i++) {
		zc = i - (i/20);
	    xc = cosLerp(TWOPI * zc / rings)* center_offset;//wrong 4096 circle!!!!
      	yc = sinLerp(TWOPI * zc / rings)* center_offset;
		for (int j = 0; j <= bands; j++) {

			float x = xc + cosLerp(j * TWOPI / bands ) * radius;
			float y = yc + sinLerp(j * TWOPI / bands ) * radius;
			float z = 0-(i*2);
